...
dp.rolling.window=function(Y,nprev,indice=1,lag=1){
  ...
  for(i in nprev:1){  #NB: backwards FOR loop: going from 180 down to 1
    ...
    fact=rundp(Y.window,indice,lag) #call the function to fit the AR(p) selected on BIC and generate h-step forecast
